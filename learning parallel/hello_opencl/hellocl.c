...
int main() 
{
	/* This is our first host buffer */
	char buf[] = "Hello, OpenCL!";

	/* Defines the number of work items, we consider each character as a work item */
	size_t worksize = strlen(buf) + 1;

	/* This is our second host buffer. It is the target of the copy operation */
	char buf2[worksize+1];
	...
	/* Initialize the target host buffer with some defined values so we are sure that
	 * something was copied */
	memset(buf2,'?',worksize);
	buf2[worksize] = 0;
    ...
    if (!(k_src = read_file_contents("example.cl")))
    {
	...
	cl_srcmem = clCreateBuffer(context, CL_MEM_READ_ONLY, worksize, NULL, &error);
	if (error != CL_SUCCESS)
	{
	 ...
	 */
	cl_destmem = clCreateBuffer(context, CL_MEM_WRITE_ONLY, worksize, NULL, &error);
	if (error != CL_SUCCESS)
	{
	...
	/* We declare that the cl_srcmem is the first parameter */
	error = clSetKernelArg(k_example, 0, sizeof(cl_srcmem), &cl_srcmem);
	if (error != CL_SUCCESS)
	{
		fprintf(stderr,"Couldn't set first argument for kernel!\n");
		goto out;
	}
	/* And we declare that cl_destmem is the second parameter */
	error = clSetKernelArg(k_example, 1, sizeof(cl_destmem), &cl_destmem);
	if (error != CL_SUCCESS)
	{
	...
	error = clEnqueueWriteBuffer(cq, cl_srcmem, CL_FALSE, 0, worksize, buf, 0, NULL, NULL);
	if (error != CL_SUCCESS)
	{
	...
	error = clEnqueueNDRangeKernel(cq, k_example, 1, NULL, &worksize, NULL, 0, NULL, NULL);
	if (error != CL_SUCCESS)
	{
	...
	error = clEnqueueReadBuffer(cq, cl_destmem, CL_FALSE, 0, worksize, buf2, 0, NULL, NULL);
	if (error != CL_SUCCESS)
	{
